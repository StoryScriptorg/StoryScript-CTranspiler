...
class Lexer:
    def __init__(
        self, symbol_table, out_file_name, file_helper=None, auto_reallocate=True
    ):
    ...
    def throw_keyword(self, tc, multiple_commands_index, ln="Unknown"):
        ...
        if tc[1] == "InvalidSyntax":
            ...
            self.raise_transpile_error(
                "InvalidValue: The Exception entered is not defined", ln
            )
    ...
    def variable_setting(self, tc, multiple_commands_index, ln):
        ...
        if tc[1] == "=":  # Set operator
            ...
            if error:
                self.raise_transpile_error(error[0], ln)
            if oldvar[2]:
                if oldvar[0] == Types.String:
                    error = self.symbol_table.set_variable(
                        tc[0], res, vartype, oldvar[2], len(res) - 1
                    )
                    if self.auto_reallocate:
                        if oldvar[3] < (len(res) - 1):
                            self.file_helper.insert_content(
                                f"{tc[0]} = realloc({tc[0]}, {len(res) - 1});"
                            )
                        else:
                        ...
                        print(
                            "INFO: To set a Message to a String, Input string must be less than the Size specified or equal the Original string size If declared with initial value."
                        )
                        if len(res) - 1 > oldvar[3]:
                            self.raise_transpile_error(
                                'InvalidValue: The input string length is more than the Original Defined size. If you want a Dynamically allocated string, Please don\'t use "--no-auto-reallocate" option.',
                                ln,
                            )
            ...
            if vartype == Types.String:
                length = len(res) - 1
                error = self.symbol_table.set_variable(
                    tc[0], res, vartype, oldvar[2], length
                )
                if length > oldvar[3]:
                    self.raise_transpile_error(
                        "InvalidValue: The input string length is more than the Original Defined size. If you want a Dynamically allocated string, Please use the Heap allocated string instead If you want to make the String dynamiccally allocated.",
                        ln,
                    )
        ...
        if oldvar[2]:
            if oldvar[0] == Types.String:
                self.raise_transpile_error(
                    f"InvalidTypeException: You cannot use {operator}= with String.", ln
                )
            if oldvar[0] == Types.Dynamic:
                self.raise_transpile_error(
                    f"InvalidTypeException: You cannot use {operator}= with Dynamics.",
                    ln,
                )
    ...
    def if_else_statement(self, tc: list, ln: int):
        ...
        def check_string_comparison(condition):
            ...
            resl = bool(
                parser.parse_type_from_value(" ".join(condition[:operator_index]))
                == Types.String
            )
            if "".join(condition[:operator_index]) in allvar:
                resl = resl or (
                    self.symbol_table.get_variable_type(
                        "".join(condition[:operator_index])
                    )
                    == Types.String
                )

            resr = bool(
                parser.parse_type_from_value(" ".join(condition[operator_index + 1 :]))
                == Types.String
            )
            if "".join(condition[operator_index + 1 :]) in allvar:
                resr = resr or (
                    self.symbol_table.get_variable_type(
                        "".join(condition[operator_index + 1 :])
                    )
                    == Types.String
                )
            ...
            if tc[0] in all_variable_name:
                ...
                if tc[0] in {
                    ...
                    try:
                        ...
                        if tc[1] == "heap":
                            ...
                            res, error = self.analyseCommand(
                                tc[4 : multiple_commands_index + 1], tc[2]
                            )
                            if isinstance(error, Exceptions):
                                self.raise_transpile_error(res, ln)
                        else:
                            res, error = self.analyseCommand(
                                tc[3 : multiple_commands_index + 1], tc[1]
                            )
                            if isinstance(error, Exceptions):
                                self.raise_transpile_error(res, ln)
                        value = ""
                        ...
                        if vartype == Exceptions.InvalidSyntax:
                            self.raise_transpile_error(
                                "InvalidSyntax: Invalid value", ln
                            )
                        res = parser.parse_escape_character(res)
                        ...
                        if isHeap:
                            outvartype = tc[0]
                            if tc[0] == "var":
                                outvartype = parser.convert_types_enum_to_string(
                                    vartype
                                )
                            if vartype == Types.String:
                                ...
                                self.symbol_table.set_variable(
                                    tc[2], res, vartype, True, len(res) - 1
                                )
                                self.file_helper.insert_content(
                                    f"char *{tc[2]} = (char*)malloc({len(res) - 1});"
                                )
                                return (
                                ...
                                if isinstance(intval, int):
                                    if intval <= 2147483647 and intval >= -2147483647:
                                        if (
                                            ...
                                            and intval >= -9223372036854775807
                                        ):
                                            bytesize = 16
                                        else:
                                            bytesize = 8
                            ...
                            self.file_helper.insert_content(
                                f"{outvartype} *{tc[2]} = ({outvartype}*)malloc(sizeof({outvartype}));"
                            )
                            return f"*{tc[2]} = {res};", ""
                        if tc[0] == "var":
                        ...
                        if vartype == Exceptions.InvalidSyntax:
                            self.raise_transpile_error(
                                "InvalidSyntax: Invalid type", ln
                            )
                        if tc[1] == "heap":
                            if vartype == Types.String:
                                self.symbol_table.set_variable(
                                    tc[2], None, vartype, True, int(tc[3])
                                )
                                if "string.h" not in libraryIncluded:
                                    libraryIncluded.append("string.h")
                            ...
                            return (
                                f"{tc[0]} *{tc[2]} = ({tc[0]}*)malloc(sizeof({tc[0]}));",
                                "",
                            )
                    ...
                    if not value.endswith(
                        ...
                        self.raise_transpile_error(
                            close_paren_needed, ln
                        )  # Return error if not exists
                    value = value[1:-1]  # Cut parentheses out of the string
                    return f'scanf("%s", &{varcontext})', {
                        int(tc[1])
                    }  # Return the Recieved Input
                elif tc[0] == "if":
                    return self.if_else_statement(tc, ln)
                elif tc[0] == "exit":
                    value = ""
                    for i in tc[
                        1 : multiple_commands_index + 1
                    ]:  # Get all parameters provided as 1 long string
                        value += i + " "
                    value = value[:-1]
                    if not value.startswith(
                    ...
                    if value.startswith('"'):
                        self.raise_transpile_error(
                            "InvalidValue: Exit code can only be integer.", ln
                        )
                    if value.endswith('"'):
                        self.raise_transpile_error(
                            "InvalidValue: Exit code can only be integer.", ln
                    ...
                    if not value:
                        self.raise_transpile_error(
                            'InvalidValue: Parameter "status" (int) required.', ln
                        )
                    ...
                    return self.throw_keyword(
                        tc, multiple_commands_index
                    )  # Go to the Throw keyword function
                elif tc[0] == "del":
                    if tc[1] not in all_variable_name:
                        self.raise_transpile_error(
                            f"NotDefinedException: The variable {tc[1]} is not defined.",
                            ln,
                        )
                    if not self.symbol_table.get_variable(tc[1])[2]:
                        self.raise_transpile_error(
                            f"InvalidValue: The variable {tc[1]} is not heap allocated.",
                            ln,
                        )
                    return f"free({tc[1]});", ""
                elif tc[0] == "loopfor":
                    try:
                        ...
                        genvarname += ascii_letters[randint(0, 51)]
                        self.file_helper.insert_content(
                            f"for (int {genvarname} = 0; {genvarname} < {int(tc[1])}; {genvarname}++)"
                            + " {"
                        )
                        self.file_helper.indent_level += 1
                        for i in commands:
                        ...
                        self.raise_transpile_error(
                            "InvalidValue: Count must be an Integer. (Whole number)", ln
                        )
                elif tc[0] == "switch":
                    return self.switch_case_statement(tc, ln)
                else:
                    self.raise_transpile_error(
                        "NotImplementedException: This feature is not implemented", ln
                    )
