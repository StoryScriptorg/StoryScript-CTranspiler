...
def parse_file(out_file, file_name, auto_reallocate=True, minified=False):
    ...
    if (
        STORYSCRIPT_INTERPRETER_DEBUG_MODE
    ):  # Check if the run mode was Debug mode or not.
    ...
    try:
        ...
        print(
            f"Cannot open file {fileName}. File does not exist."
        )  # Print the error and terminate the function If the file does not exist.
        return
    ...
    for i in tqdm(lines, ncols=75):
        ...
        try:
            if minified and len(commands) != 0 and commands[0] == "//":
                continue
            lexer.file_helper.insert_content(
                lexer.analyseCommand(commands, ln=line_index)[0]
            )
        except Exception:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Transpile StoryScript code into C code."
    )
    parser.add_argument("-i", "--input", help="The input file.", required=True)
    parser.add_argument("-o", "--output", help="The target output file.", required=True)
    parser.add_argument(
        "-no-realloc",
        "--no-auto-reallocate",
        action="store_true",
